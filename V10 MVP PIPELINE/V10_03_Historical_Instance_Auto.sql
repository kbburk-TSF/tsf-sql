-- V10_03_Historical_Instance_Auto.sql
-- Generated: 2025-09-22T15:04:07Z
-- PURPOSE: Phase 1 (Historical Instance) autonomous pipeline aligned to V10 Core Schema.
-- Rules:
--   * NO search_path changes. NO REVOKE statements.
--   * NO math changes to historical instance logic; still two-pass (hydrate then compute).
--   * NO ARG dependencies for later pipeline phases.
-- Tracking model:
--   * Drops any dependency on forecast_registry.status (legacy).
--   * Uses forecast_registry.pipeline_status only; sets it to 'historical_ready' when finished.
--   * register_forecast() upserts name/filename only (no status writes).
-- Compatible with: V9_03 (math) + V10_01+ core schema updates (new tracking).
...
-- =====================================================================
-- CORE: build_instance_historical_core (VERBATIM MATH; environment guards)
-- =====================================================================
...
BEGIN
  ...
  -- PASS 1: Hydrate raw rows from staging; reset computed fields
  ...
  -- PASS 2: Compute qmv/mmv and LQM/LMM (unchanged math)
  WITH rows AS (
    SELECT ih.forecast_id, ih."date" AS d, ih.value,
  ...
  bounds AS (
    ...
    FROM rows r GROUP BY r.forecast_id
  ),
  qagg AS (
  ...
  q_rank AS (
    SELECT ih.forecast_id, date_trunc('quarter', ih."date")::date AS qprev, ih.value AS v,
           row_number() OVER (PARTITION BY ih.forecast_id, date_trunc('quarter', ih."date") ORDER BY ih."date" DESC) AS rn
    FROM engine.instance_historical ih
    WHERE ih.forecast_id = p_forecast_id AND ih.value IS NOT NULL
  ),
  lqm AS (
    SELECT forecast_id, qprev,
    ...
    FROM q_rank GROUP BY forecast_id, qprev
  ),
  m_rank AS (
    SELECT ih.forecast_id, date_trunc('month', ih."date")::date AS mprev, ih.value AS v,
           row_number() OVER (PARTITION BY ih.forecast_id, date_trunc('month', ih."date") ORDER BY ih."date" DESC) AS rn
    FROM engine.instance_historical ih
    WHERE ih.forecast_id = p_forecast_id AND ih.value IS NOT NULL
  ),
  lmm AS (
    SELECT forecast_id, mprev,
    ...
    FROM m_rank GROUP BY forecast_id, mprev
  ),
  calc AS (
    SELECT r.d,
...
-- =====================================================================
-- AUDIT WRAPPER (explicit ids) – aligns to V10 instance_runs
-- =====================================================================
...
BEGIN
  ...
  VALUES (p_run_id, p_forecast_id, 'running', now(), now())
  ON CONFLICT (run_id) DO UPDATE
    SET status='running', started_at=now(), overall_error=NULL;
...
-- =====================================================================
-- Registry Updater (NO legacy 'status' writes)
-- =====================================================================
CREATE OR REPLACE FUNCTION engine.register_forecast(p_forecast_id uuid)
RETURNS void
LANGUAGE plpgsql
AS $$
DECLARE
  _fname text;
BEGIN
  SELECT max(forecast_name) INTO _fname
  FROM engine.staging_historical
  WHERE forecast_id = p_forecast_id;

  INSERT INTO engine.forecast_registry (
    forecast_id, forecast_name, source_csv_filename, created_at, updated_at
  )
  VALUES (p_forecast_id, COALESCE(_fname,'unknown'), _fname, now(), now())
...
-- =====================================================================
-- Delete staging rows for a forecast_id (unchanged logic)
-- =====================================================================
...
-- =====================================================================
-- Triggered pipeline: AFTER INSERT on staging_historical
--   1) register_forecast (no status writes)
--   2) build_instance_historical_core
--   3) delete_staging_for
--   4) registry.pipeline_status := 'historical_ready'
-- =====================================================================
CREATE OR REPLACE FUNCTION engine._process_staging_to_historical(p_forecast_id uuid)
RETURNS void
LANGUAGE plpgsql
AS $$
BEGIN
  PERFORM pg_advisory_xact_lock(hashtext(p_forecast_id::text));

  PERFORM engine.register_forecast(p_forecast_id);
  PERFORM engine.build_instance_historical_core(p_forecast_id);
  PERFORM engine.delete_staging_for(p_forecast_id);

  UPDATE engine.forecast_registry
     SET pipeline_status = 'historical_ready',
         updated_at = now()
   WHERE forecast_id = p_forecast_id;
END;
$$;

CREATE OR REPLACE FUNCTION engine.trg_staging_to_historical_stmt()
RETURNS trigger
LANGUAGE plpgsql
AS $$
DECLARE
  _fid uuid;
BEGIN
  FOR _fid IN SELECT DISTINCT forecast_id FROM new_rows WHERE forecast_id IS NOT NULL LOOP
    PERFORM engine._process_staging_to_historical(_fid);
  END LOOP;
  RETURN NULL;
END;
$$;

DROP TRIGGER IF EXISTS trg_staging_to_historical_stmt ON engine.staging_historical;

CREATE TRIGGER trg_staging_to_historical_stmt
AFTER INSERT ON engine.staging_historical
REFERENCING NEW TABLE AS new_rows
FOR EACH STATEMENT
EXECUTE FUNCTION engine.trg_staging_to_historical_stmt();

-- Grants (app/reader adjust as needed)
GRANT EXECUTE ON FUNCTION engine.build_instance_historical() TO aq_engine_owner;
GRANT EXECUTE ON FUNCTION engine.build_instance_historical_run(uuid, uuid) TO aq_engine_owner;
GRANT EXECUTE ON FUNCTION engine.build_instance_historical_core(uuid) TO aq_engine_owner;
GRANT EXECUTE ON FUNCTION engine.register_forecast(uuid) TO aq_engine_owner;
GRANT EXECUTE ON FUNCTION engine.delete_staging_for(uuid) TO aq_engine_owner;
GRANT EXECUTE ON FUNCTION engine._process_staging_to_historical(uuid) TO aq_engine_owner;
GRANT EXECUTE ON FUNCTION engine.trg_staging_to_historical_stmt() TO aq_engine_owner;
