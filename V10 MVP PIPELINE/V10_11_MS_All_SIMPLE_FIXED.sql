-- V10_11_MS_All_SIMPLE_FIXED.sql
-- Generated: 2025-09-23 00:44
-- FIX: After calling build_forecast_ms_core, verify rows were written for the claimed forecast_id
--      before flipping ms_complete='complete'. If zero rows found, reset to NULL and append error.
--      No changes to forecast logic; wrapper only.
BEGIN;

CREATE OR REPLACE FUNCTION engine.V10_11_MS_All_SIMPLE_FIXED()
RETURNS void
LANGUAGE plpgsql
AS $$
DECLARE
...
BEGIN
  ...
  LOOP
    ...
    -- Verify: rows written for this forecast_id in any ms table
    FOR t IN
      SELECT format('%I.%I','engine',tablename) AS qname
      FROM pg_catalog.pg_tables
      WHERE schemaname='engine' AND tablename LIKE '%\_instance\_forecast\_ms' ESCAPE '\'
    LOOP
      ...
      UPDATE engine.forecast_registry
         SET ms_complete=NULL,
             ...
             overall_error = COALESCE(overall_error,'') || CASE WHEN overall_error IS NULL THEN '' ELSE E'\n' END
                             || '['||to_char(now(),'YYYY-MM-DD"T"HH24:MI:SSOF')||'] Wrapper: no ms rows written'
       WHERE forecast_id=rec_id;
    END IF;
...
GRANT EXECUTE ON FUNCTION engine.V10_11_MS_All_SIMPLE_FIXED() TO aq_engine_owner;
