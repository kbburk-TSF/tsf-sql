...
-- Views
CREATE OR REPLACE VIEW engine.v_instance_run_phases AS
SELECT rp.run_id, rp.forecast_id, rp.phase, rp.status,
       rp.started_at, rp.finished_at, rp.rows_written, rp.message,
       rp.created_at, rp.updated_at
FROM engine.instance_run_phases rp;

CREATE OR REPLACE VIEW engine.v_instance_run_latest AS
WITH ranked AS (
  SELECT ir.*,
         ROW_NUMBER() OVER (
           PARTITION BY ir.forecast_id
           ORDER BY GREATEST(COALESCE(ir.started_at, '-infinity'::timestamptz),
                             COALESCE(ir.finished_at, '-infinity'::timestamptz)) DESC,
                    ir.run_id DESC
         ) AS rn
  FROM engine.instance_runs ir
)
SELECT * FROM ranked WHERE rn = 1;

CREATE OR REPLACE VIEW engine.v_instance_run_status AS
SELECT l.forecast_id, l.run_id,
       l.status AS run_status, l.started_at AS run_started_at, l.finished_at AS run_finished_at,
       p.phase, p.status AS phase_status, p.started_at AS phase_started_at,
       p.finished_at AS phase_finished_at, p.rows_written, p.message
FROM engine.v_instance_run_latest l
JOIN engine.instance_run_phases p ON p.run_id = l.run_id;

CREATE OR REPLACE VIEW engine.v_run_status_latest_all AS
SELECT s.forecast_id, s.run_id, s.run_status,
       s.run_started_at, s.run_finished_at, s.phase,
       CASE s.phase
         WHEN 'sr_all' THEN 1
         WHEN 'fc_ms_all' THEN 2
         WHEN 'fc_msq_all' THEN 3
         WHEN 'fc_msqm_all' THEN 4
         WHEN 'sr_s' THEN 101 WHEN 'sr_sq' THEN 102 WHEN 'sr_sqm' THEN 103
         WHEN 'fc_ms' THEN 104 WHEN 'fc_msq' THEN 105 WHEN 'fc_msqm' THEN 106
         ELSE 999 END AS phase_order,
       s.phase_status, s.phase_started_at, s.phase_finished_at,
       s.rows_written, s.message
FROM engine.v_instance_run_status s;
