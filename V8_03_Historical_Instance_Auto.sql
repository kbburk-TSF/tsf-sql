-- V6_04_Historical_Instance_Auto.sql
-- ==============================================================================================
-- VERSION CONTROL
-- V6.04.0 (2025-09-18)
-- PURPOSE:
--   * ONE FILE to set up Phase 1 (Historical Instance) autonomous pipeline.
--   * Includes the canonical historical instance functions (VERBATIM from V5_04).
--   * Adds a dedicated forecast_registry updater and staging purge function.
--   * Wires an AFTER INSERT (statement-level) trigger on engine.staging_historical to:
--       1) engine.register_forecast(forecast_id)
--       2) engine.build_instance_historical_core(forecast_id)
--       3) engine.delete_staging_for(forecast_id)
--   * No SR/Forecast orchestration in this file.
-- ==============================================================================================

-- >>> BEGIN INCLUDED (VERBATIM): V5_04_HistoricalInstanceTable.sql >>>
...
-- <<< END INCLUDED: V5_04_HistoricalInstanceTable.sql <<<

-- >>> BEGIN NEW: Forecast Registry Updater >>>

-- Registry updater (new in V6_04) ---------------------------------------------------------------
-- Upserts forecast_registry with filename and 'staged' status for the given forecast_id.
BEGIN;
...
BEGIN
  ...
  VALUES (p_forecast_id, COALESCE(_fname,'unknown'), _fname, 'staged', now(), now())
...
-- <<< END NEW: Forecast Registry Updater <<<

-- >>> BEGIN INCLUDED (VERBATIM): V3_05_DeleteStaging.sql >>>
...
-- <<< END INCLUDED: V3_05_DeleteStaging.sql <<<

-- >>> BEGIN NEW: Triggers & Pipeline >>>

-- Triggered pipeline (AFTER INSERT on staging_historical) ---------------------------------------
BEGIN;

-- Internal worker to process a single forecast_id after upload completes
CREATE OR REPLACE FUNCTION engine._process_staging_to_historical(p_forecast_id uuid)
RETURNS void
LANGUAGE plpgsql
AS $$
BEGIN
  -- Per-forecast guard against concurrent duplicate runs
  PERFORM pg_advisory_xact_lock(hashtext(p_forecast_id::text));

  -- 1) Registry upsert -> 'staged'
  PERFORM engine.register_forecast(p_forecast_id);

  -- 2) Build historical instance (canonical core)
  PERFORM engine.build_instance_historical_core(p_forecast_id);

  -- 3) Purge staging rows
  PERFORM engine.delete_staging_for(p_forecast_id);

  -- 4) Optionally mark registry as 'historical_ready'
  UPDATE engine.forecast_registry
     SET status = 'historical_ready', updated_at = now()
   WHERE forecast_id = p_forecast_id;
END;
$$;

-- Statement-level trigger using transition table to ensure we fire once per upload statement
...
-- Minimal grants for app/reader roles
GRANT EXECUTE ON FUNCTION engine.register_forecast(uuid) TO matrix_reader, tsf_engine_app;
GRANT EXECUTE ON FUNCTION engine._process_staging_to_historical(uuid) TO matrix_reader, tsf_engine_app;
GRANT EXECUTE ON FUNCTION engine.trg_staging_to_historical_stmt() TO matrix_reader, tsf_engine_app;

COMMIT;

-- END V6_04_Historical_Instance_Auto.sql

-- <<< END NEW: Triggers & Pipeline <<<
