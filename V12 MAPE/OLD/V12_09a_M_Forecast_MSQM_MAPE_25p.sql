-- VERSION: 2025-09-24 — Added name-based wrappers & catch-up for MSQM
-- MAPE CONVERSION (2025-09-24) — MAE→MAPE across columns & logic; 25p bands
-- File: V12_09_M_Forecast_MSQM_MAPE_25p.sql
-- Changes:
...
--   • New entry: engine.build_forecast_msqm_by_name(forecast_name text)
--   • Advisory lock per (forecast_id, 'MSQM')
--   • Registry upsert/update on success
--   • Catch-up: engine.build_forecast_msqm_catchup()
-- -------------------------------------------------------------------

CREATE OR REPLACE FUNCTION engine.build_forecast_msqm_by_name(forecast_name text)
RETURNS void
LANGUAGE plpgsql
AS $$
DECLARE
...
CREATE OR REPLACE FUNCTION engine.build_forecast_msqm_catchup()
RETURNS void
LANGUAGE plpgsql
AS $$
DECLARE
...
BEGIN
  FOR r IN
    ...
    FROM engine.forecast_registry fr
    WHERE fr.msqm_complete IS NULL
  LOOP
    PERFORM engine.build_forecast_msqm_by_name(r.forecast_name);
  END LOOP;
END;
$$;
