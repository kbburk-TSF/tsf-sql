-- VERSION: 2025-09-24 — Added name-based wrappers & catch-up for MSQ
-- MAPE CONVERSION (2025-09-24) — MAE→MAPE across columns & logic; 25p bands
-- File: V12_08_M_Forecast_MSQ_MAPE_25p.sql
-- Changes:
--   • Rename fv_mape→fv_mape; fv_mean_mape→fv_mean_mape; fv_mean_mape_c→fv_mean_mape_c
--   • Rename mape_comparison→mape_comparison; mean_mape_comparison→mean_mape_comparison
--   • Season metric now MAPE: average of per-row |value - fv| / |value|, excluding rows where value=0
--   • Bands: fv_u = fv + ((fv * fv_mean_mape) * 1.25); fv_l = GREATEST(0, fv - ((fv * fv_mean_mape) * 1.25))
--   • Clamp fv_mape & fv_mean_mape to 4dp

-- 3x WIDEN BAND (2025-09-24) — fv_u/l = fv ± (3.0 × fv_mean_mape). Only this change.
-- File: V11_08_M_Forecast_MSQ_3x.sql

-- 25p WIDEN BAND (2025-09-24) — fv_u/l = fv ± (1.25 × fv_mean_mape). Only this change.
-- File: V11_08_M_Forecast_MSQ_25p.sql

-- ALT CLEAN (2025-09-22) — Entire UPSERT clause removed; now DO NOTHING. Only this change.
-- File: V10_08_M_Forecast_MSQ_FIXED_ALT_CLEAN.sql
...
-- Replace placeholder fv_error with computed fv_error2 (computed at CTAS to avoid a full-table UPDATE)
    ...
    EXECUTE format($u$
      WITH m AS (
        ...
        FROM %3$s
        WHERE value IS NOT NULL AND fv IS NOT NULL AND ABS(value) > 0
    ...
    EXECUTE format($u$
      UPDATE %1$s
         SET fv_u = fv + ((fv * fv_mean_mape) * 1.25),
             fv_l = GREATEST(0, fv - ((fv * fv_mean_mape) * 1.25))
...
-- -------------------------------------------------------------------
-- VERS 2025-09-24: Add forecast-name overloads + registry writeback + lock
--   • New entry: engine.build_forecast_msq_by_name(forecast_name text)
--   • Advisory lock per (forecast_id, 'MSQ')
--   • Registry upsert/update on success
--   • Catch-up: engine.build_forecast_msq_catchup()
-- -------------------------------------------------------------------

CREATE OR REPLACE FUNCTION engine.build_forecast_msq_by_name(forecast_name text)
RETURNS void
LANGUAGE plpgsql
AS $$
DECLARE
...
BEGIN
  ...
  WHERE fr.forecast_name = forecast_name
  GROUP BY fr.forecast_id
  HAVING COUNT(*) >= 1;

  IF v_id IS NULL THEN
    RAISE EXCEPTION 'No forecast_id found for name=%', forecast_name;
  END IF;

  -- Per-forecast advisory lock (class key derived from model string)
  v_lock := ('MSQ'::text)::regclass::oid;  -- stable key per model
  PERFORM pg_advisory_lock(v_lock, ('x'||replace(v_id::text,'-',''))::bit(64)::bigint);

  BEGIN
    -- Run existing builder (best effort global run)
    PERFORM engine.build_forecast_msq();

    -- Registry writeback: mark the specific forecast/model as updated
    INSERT INTO engine.forecast_registry (forecast_id, forecast_name, updated_at, msq_complete)
    VALUES (v_id, forecast_name, now(), 'complete')
    ON CONFLICT (forecast_id)
    DO UPDATE SET updated_at = EXCLUDED.updated_at,
                  msq_complete = 'complete';

  EXCEPTION WHEN OTHERS THEN
    -- Release lock and bubble up
    PERFORM pg_advisory_unlock(v_lock, ('x'||replace(v_id::text,'-',''))::bit(64)::bigint);
    RAISE;
  END;
...
CREATE OR REPLACE FUNCTION engine.build_forecast_msq_catchup()
RETURNS void
LANGUAGE plpgsql
AS $$
DECLARE
...
BEGIN
  FOR r IN
    SELECT DISTINCT fr.forecast_name
    FROM engine.forecast_registry fr
    WHERE fr.msq_complete IS NULL
  LOOP
    PERFORM engine.build_forecast_msq_by_name(r.forecast_name);
  END LOOP;
END;
$$;
