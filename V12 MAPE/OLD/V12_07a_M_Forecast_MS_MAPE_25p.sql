-- VERSION: 2025-09-24 — Added name-based wrappers & catch-up for MS
-- MAPE CONVERSION (2025-09-24) — MAE→MAPE across columns & logic; 25p bands
-- File: V12_07_M_Forecast_MS_MAPE_25p.sql
-- Changes:
...
--   • New entry: engine.build_forecast_ms_by_name(forecast_name text)
--   • Advisory lock per (forecast_id, 'MS')
--   • Registry upsert/update on success
--   • Catch-up: engine.build_forecast_ms_catchup()
-- -------------------------------------------------------------------

CREATE OR REPLACE FUNCTION engine.build_forecast_ms_by_name(forecast_name text)
RETURNS void
LANGUAGE plpgsql
AS $$
DECLARE
...
CREATE OR REPLACE FUNCTION engine.build_forecast_ms_catchup()
RETURNS void
LANGUAGE plpgsql
AS $$
DECLARE
...
BEGIN
  FOR r IN
    ...
    FROM engine.forecast_registry fr
    WHERE fr.ms_complete IS NULL
  LOOP
    PERFORM engine.build_forecast_ms_by_name(r.forecast_name);
  END LOOP;
END;
$$;
