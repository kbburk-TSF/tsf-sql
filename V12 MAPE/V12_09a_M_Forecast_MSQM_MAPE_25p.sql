...
RAISE NOTICE 'COMPLETE series: % (elapsed %.3f s)',
  ...
  -- Mark completion in forecast_registry (msqm_completed) — type-aware
  BEGIN
    PERFORM 1
    FROM information_schema.columns
    WHERE table_schema='engine' AND table_name='forecast_registry' AND column_name='msqm_completed';
    ...
    IF FOUND THEN
      ...
      IF FOUND THEN
        EXECUTE format('UPDATE engine.forecast_registry SET msqm_completed = now() WHERE forecast_id = $1') USING latest_id;
      ELSE
        -- boolean
        PERFORM 1 FROM information_schema.columns
          WHERE table_schema='engine' AND table_name='forecast_registry' AND column_name='msqm_completed' AND data_type IN ('boolean');
        IF FOUND THEN
          EXECUTE format('UPDATE engine.forecast_registry SET msqm_completed = true WHERE forecast_id = $1') USING latest_id;
        ELSE
          -- default to text
          EXECUTE format('UPDATE engine.forecast_registry SET msqm_completed = %s WHERE forecast_id = $1', quote_literal('complete')) USING latest_id;
        END IF;
      END IF;
    END IF;
  EXCEPTION WHEN OTHERS THEN
    RAISE NOTICE 'Registry update for msqm_completed failed: %', SQLERRM;
  END;
