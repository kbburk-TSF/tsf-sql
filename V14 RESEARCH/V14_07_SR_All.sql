...
-- 2025-10-04 V14_07_SR_All: V14 hotfix — add SR preflight (build_sr_series_s/sq/sqm) so dest SR tables are created before cores run. No other changes.
-- 2025-10-04 V14_07_SR_All: Non-recursive SR preflight added (GUC-guarded). Fixes infinite loop. No other changes.
BEGIN;
...
BEGIN
  ...
  -- PRE-FLIGHT: Ensure SR destination tables exist (wrappers create schema safely)
  -- PRE-FLIGHT: SAFE (non-recursive). Creates SR tables once if missing.
  IF current_setting('engine.sr_preflight', true) IS DISTINCT FROM '1' THEN
    PERFORM set_config('engine.sr_preflight','1', true);
    BEGIN
      IF to_regclass('engine.s_instance_sr_s')   IS NULL THEN PERFORM engine.build_sr_series_s();   END IF;
      IF to_regclass('engine.s_instance_sr_sq')  IS NULL THEN PERFORM engine.build_sr_series_sq();  END IF;
      IF to_regclass('engine.s_instance_sr_sqm') IS NULL THEN PERFORM engine.build_sr_series_sqm(); END IF;
    EXCEPTION WHEN OTHERS THEN
      RAISE NOTICE 'SR preflight skipped: %', SQLERRM;
    END;
    PERFORM set_config('engine.sr_preflight','0', true);
  END IF;
