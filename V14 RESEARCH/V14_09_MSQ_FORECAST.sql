-- 2025-10-05 HOTFIX-18: Add missing 'USING latest_id' to PASS 7 flags/stats UPDATE to satisfy $1.
...
-- 2025-10-05 HOTFIX-16: PASS 7 stability — remove stray USING params on two UPDATEs (no $ placeholders),
-- and make PASS 7b variability UPDATE respect dynamic season column (no hard-coded 'season').
...
-- 2025-10-05 HOTFIX-18: Fix doubled quotes ''A0'' → 'A0' inside dollar-quoted EXECUTEs to resolve syntax error near A0.

-- 2025-10-05 HOTFIX-17: PASS 7 placeholder fix — replace lone $2 params with $1 and drop the dummy NULL in USING.
...
ANALYZE __tmp_forecast_build;
    ...
    EXECUTE format($u$ UPDATE %1$s
     ...
     WHERE forecast_id = $1 AND value IS NOT NULL AND fv IS NOT NULL AND ABS(value) > 0 $u$, dest_qual) USING latest_id;
    ...
    EXECUTE format($u$ UPDATE %1$s SET
        ...
        WHERE forecast_id = $1 $u$, dest_qual) USING latest_id;
...
EXECUTE 'DROP TABLE IF EXISTS __a0_map';
    ...
    EXECUTE format($u$ UPDATE %2$s t
      ...
      FROM __a0_map a0
     WHERE t.base_model = a0.base_model AND t.%1$I = a0.yqm $u$, dest_season_col, dest_qual) ;
...
WITH j AS (
      ...
      UPDATE %1$s t
        ...
        FROM cum c
       WHERE t.model_name = c.model_name AND t.%2$I = c.yqm
$v$, dest_qual, dest_season_col);
