...
-- Purpose: MSQ forecast core per CANNON spec with destination schema creation (1:1 column set/types).
-- Notes:
--  - Identical to MSQM variant except all references to msqm → msq.
-- ===============================================================================================
...
BEGIN
    ...
    WHERE fr.forecast_name = msq_forecast.forecast_name
    ...
    PERFORM engine.msq_forecast__core(latest_id, forecast_name);
END;
$$;

CREATE OR REPLACE FUNCTION engine.msq_forecast__core(latest_id uuid, forecast_name text)
RETURNS void
LANGUAGE plpgsql
AS $$
DECLARE
