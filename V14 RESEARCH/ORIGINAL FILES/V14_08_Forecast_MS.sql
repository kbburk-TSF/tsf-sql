...
-- Purpose: MS forecast core per CANNON spec with destination schema creation (1:1 column set/types).
-- Notes:
--  - Identical to MSQM variant except all references to msqm → ms.
-- ===============================================================================================
...
BEGIN
    ...
    WHERE fr.forecast_name = ms_forecast.forecast_name
    ...
    PERFORM engine.ms_forecast__core(latest_id, forecast_name);
END;
$$;

CREATE OR REPLACE FUNCTION engine.ms_forecast__core(latest_id uuid, forecast_name text)
RETURNS void
LANGUAGE plpgsql
AS $$
DECLARE
