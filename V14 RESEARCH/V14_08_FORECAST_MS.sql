...
--              Wrapper signatures and names unchanged: engine.ms_forecast(text), engine.ms_forecast__core().
...
-- Entry point: engine.ms_forecast(forecast_name TEXT) — preserved from V13
...
-- Write PASS 1 rows into work table (map generic 'msr' -> dynamic <base>_msr column)
  ...
  -- Mark completion in forecast_registry (ms_complete)
  BEGIN
    PERFORM 1
