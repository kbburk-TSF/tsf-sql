-- ===================================================================
-- File: V13_14_TSF_Full_Views_Classical.sql
-- Updated: 2025-09-25
-- CHANGE: Keep all logic intact. At the final global view (engine.tsf_vw_full),
--         append ARIMA_M, SES_M, HWES_M by joining engine.instance_historical
--         ON (forecast_id, date) and copying ih.arima_m, ih.ses_m, ih.hwes_m.
--         No other changes.
-- ===================================================================
    ...
    LOOP
        ...
        v_sql := format($v$
            ...
            FROM (
                ...
                FROM %I.%I t
                WHERE t.fv IS NOT NULL AND t.fv <> 0 AND t.fv_mean_mape_c >= 5 AND t.fv_mean_mape IS NOT NULL
            ) AS t_w
            WHERE t_w._rn = 1;
...
BEGIN
    ...
    v_ins_sql := format($SQL$
        ...
        FROM (
            ...
            FROM ( %s ) AS y WHERE y.fv IS NOT NULL AND y.fv <> 0 AND y.fv_mean_mape_c >= 5
        ) g
...
-- FINAL VIEW change: append three columns via join to instance_historical
...
SELECT
    ...
    ih.ses_m   AS "SES_M",
    ih.hwes_m  AS "HWES_M"
FROM engine.tsf_vw_full_cache c
JOIN engine.instance_historical ih
  ON ih.forecast_id = c.forecast_id
 AND ih.date        = c.date;
