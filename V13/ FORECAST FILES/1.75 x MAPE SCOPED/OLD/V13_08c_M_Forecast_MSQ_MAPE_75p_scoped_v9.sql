-- 2025-09-26 SCOPED v9: Fix temp swap scope; all v8 optimizations retained; no math/logic changes.
...
BEGIN
  ...
  latest_id := forecast_target_id;
    -- v9 prelude: per-run temp subsets (scoped to latest_id) + stats and compact date domain
    ...
    CREATE TEMP TABLE __dates ON COMMIT DROP AS
    SELECT DISTINCT date FROM __ih_subset;
    CREATE INDEX ON __dates (date);

    ANALYZE __ih_subset;
    ANALYZE __ms_subset;
    ANALYZE __msq_subset;
    ANALYZE __msqm_subset;
    ANALYZE __dates;
...
-- Replace placeholder fv_error with computed fv_error2 (computed at CTAS to avoid a full-table UPDATE)
    ...
    EXECUTE format($u$ WITH m AS (
      ...
      UPDATE %3$s t
         SET fv_mape = m.mape
        FROM m WHERE t.forecast_id = $1 AND t.date IN (SELECT date FROM __dates) AND  t.%1$I       = m.series
    ...
    EXECUTE format($u$ WITH stats AS (
      ...
      UPDATE %1$s t
        ...
        FROM stats st WHERE t.forecast_id = $1 AND t.date IN (SELECT date FROM __dates) AND  t.%2$I       = st.series
    ...
    EXECUTE format($u$ UPDATE %1$s
         SET fv_u = fv + ((fv * fv_mean_mape) * 1.75),
             fv_l = GREATEST(0, fv - ((fv * fv_mean_mape) * 1.75)) WHERE forecast_id = $1 AND date IN (SELECT date FROM __dates) AND  fv_mean_mape IS NOT NULL $u$, dest_qual) USING latest_id;
    GET DIAGNOSTICS rcnt = ROW_COUNT;
    IF enable_full_analyze THEN EXECUTE format('ANALYZE %s', dest_qual); END IF;
         ...
         SET
           ...
           accuracy_comparison =
             CASE
               ...
               THEN 'Y' ELSE 'N'
             END
        FROM __a0_map a0 WHERE t.forecast_id = $1 AND t.date IN (SELECT date FROM __dates) AND  t.base_model = a0.base_model
    ...
    EXECUTE format($u$ WITH flags AS (
      ...
      UPDATE %3$s t
        ...
        FROM stats st WHERE t.forecast_id = $1 AND t.date IN (SELECT date FROM __dates) AND  t.%1$I       = st.series
         AND t.model_name = st.model_name
         AND t.%2$I       = st.yqm
         AND (
    ...
    EXECUTE format($u$ WITH stats AS (
      ...
      UPDATE %3$s t
        ...
        FROM ints_p ip WHERE t.forecast_id = $1 AND t.date IN (SELECT date FROM __dates) AND  t.%1$I       = ip.series
    ...
    EXECUTE format($c$ UPDATE %s SET
        ...
        fv_variance_mean = CASE WHEN fv_variance_mean IS NULL THEN NULL ELSE round(fv_variance_mean::numeric, 4) END $c$, dest_qual);
      ...
      EXECUTE format($w$ UPDATE %s SET
        ...
        WHERE
          ...
          OR (fv_variance_mean IS NOT NULL AND fv_variance_mean IS DISTINCT FROM round(fv_variance_mean::numeric, 4)) $w$, dest_qual);
      GET DIAGNOSTICS rcnt = ROW_COUNT;
    IF enable_full_analyze THEN EXECUTE format('ANALYZE %s', dest_qual); END IF;
