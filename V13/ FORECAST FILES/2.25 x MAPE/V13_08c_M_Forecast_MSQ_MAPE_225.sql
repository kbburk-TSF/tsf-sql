-- 2025-09-29: MAPE band multiplier set to 2.25 (was 1.75). No other changes.
-- 2025-09-26 HOTFIX-10: removed all lines containing 'temp_buffers' (comments included).
-- 2025-09-27: Scope lock — removed __msqm_subset handling; normalized index name to _sq_yqm; dropped __work s_qm index.
-- 2025-09-26 HOTFIX-9: move __work creation after dest_qual END IF and guard columns; remove early block.
-- 2025-09-26 HOTFIX-8: use string concatenation for LIKE identifier (dest_qual) to avoid parser issues.
-- 2025-09-26 HOTFIX-7: LIKE uses dest_qual (already schema-qualified/quoted) instead of to_regclass.
-- 2025-09-26 HOTFIX-6: eliminate remaining 'AS SELECT * FROM %s WHERE false' variants.
-- 2025-09-26 HOTFIX-5: use CREATE TEMP TABLE __work (LIKE dest INCLUDING ALL); remove SELECT * FROM % dest WHERE false.
-- 2025-09-26 HOTFIX-4: guard __work column indexes behind pg_attribute checks to avoid 'column s does not exist'.
-- 2025-09-26 SCOPED v11: Dest writes gated by (forecast_id, date) via __keys; strict scoping; no math/logic changes.
-- 2025-09-26 SCOPED v12-perf2: __universe + __work, planner nudges, drop-if-exists, extra __work indexes+stats, ANALYZE-after-writes, pre-filter subsets; no logic changes.
...
-- Entry point: engine.msq_forecast(forecast_name TEXT)
...
BEGIN
  ...
  PERFORM engine.msq_forecast__core();
...
-- v12 perf: additional function-local settings
...
-- v12 perf2: pre-filter subsets by __universe (in-place)
BEGIN
IF to_regclass('__ih_subset') IS NOT NULL THEN
    EXECUTE 'CREATE TEMP TABLE __ih_subset_filtered ON COMMIT DROP AS SELECT s.* FROM __ih_subset s JOIN __universe u USING (forecast_id, date)';
    EXECUTE 'DROP TABLE __ih_subset';
    EXECUTE 'ALTER TABLE __ih_subset_filtered RENAME TO __ih_subset';
    EXECUTE 'CREATE INDEX ON __ih_subset (forecast_id, date)';
    EXECUTE 'ANALYZE __ih_subset';
  END IF;
  IF to_regclass('__ms_subset') IS NOT NULL THEN
    EXECUTE 'CREATE TEMP TABLE __ms_subset_filtered ON COMMIT DROP AS SELECT s.* FROM __ms_subset s JOIN __universe u USING (forecast_id, date)';
    EXECUTE 'DROP TABLE __ms_subset';
    EXECUTE 'ALTER TABLE __ms_subset_filtered RENAME TO __ms_subset';
    EXECUTE 'CREATE INDEX ON __ms_subset (forecast_id, date)';
    EXECUTE 'ANALYZE __ms_subset';
  END IF;
  IF to_regclass('__msq_subset') IS NOT NULL THEN
    EXECUTE 'CREATE TEMP TABLE __msq_subset_filtered ON COMMIT DROP AS SELECT s.* FROM __msq_subset s JOIN __universe u USING (forecast_id, date)';
    EXECUTE 'DROP TABLE __msq_subset';
    EXECUTE 'ALTER TABLE __msq_subset_filtered RENAME TO __msq_subset';
    EXECUTE 'CREATE INDEX ON __msq_subset (forecast_id, date)';
    EXECUTE 'ANALYZE __msq_subset';
  END IF;
END;
...
END IF;
    ...
    -- v12 FIX-FINAL: create work table mirror *after* dest_qual exists; redirect IO to __work
    ...
    IF EXISTS (SELECT 1 FROM pg_attribute WHERE attrelid='__work'::regclass AND attname='s' AND NOT attisdropped) THEN
      EXECUTE 'CREATE INDEX IF NOT EXISTS __work_s_idx ON __work (s)';
    END IF;
    IF EXISTS (SELECT 1 FROM pg_attribute WHERE attrelid='__work'::regclass AND attname='s_q' AND NOT attisdropped) THEN
      EXECUTE 'CREATE INDEX IF NOT EXISTS __work_sq_idx ON __work (s_q)';
    END IF;
IF EXISTS (SELECT 1 FROM pg_attribute WHERE attrelid='__work'::regclass AND attname='s_yqm' AND NOT attisdropped) THEN
      EXECUTE 'CREATE INDEX IF NOT EXISTS __work_syqm_idx ON __work (s_yqm)';
    END IF;
...
-- Replace placeholder fv_error with computed fv_error2 (computed at CTAS to avoid a full-table UPDATE)
    ...
    ------------------------------------------------------------------
    t_pass_start := clock_timestamp();
    RAISE NOTICE '[%] PASS 3 — fv_mape ', t_pass_start;
    ...
    ------------------------------------------------------------------
    -- PASS 4a — fv_mean_mape & fv_mean_mape_c (strict chronological lag by season_start; exclude current)
    ------------------------------------------------------------------
    t_pass_start := clock_timestamp();
    ...
    EXECUTE format($u$ UPDATE %1$s
         SET fv_u = fv + ((fv * fv_mean_mape) * 2.25),
             fv_l = GREATEST(0, fv - ((fv * fv_mean_mape) * 2.25))
    ...
    ------------------------------------------------------------------
    EXECUTE format('CREATE INDEX IF NOT EXISTS %I ON %s (%I, model_name, %I)', dest_rel || '_sq_yqm', dest_qual, dest_series_col, dest_season_col);
    IF enable_full_analyze THEN EXECUTE format('ANALYZE %s', dest_qual); END IF;
    ------------------------------------------------------------------
    ...
    EXECUTE format($c$ UPDATE %s SET
      ...
      EXECUTE format($w$ UPDATE %s SET
        ...
        WHERE
          ...
          OR (fv_mape          IS NOT NULL AND fv_mape          IS DISTINCT FROM round(fv_mape::numeric, 4))
          OR (fv_mean_mape     IS NOT NULL AND fv_mean_mape     IS DISTINCT FROM round(fv_mean_mape::numeric, 4))
          ...
          OR (fv_variance_mean IS NOT NULL AND fv_variance_mean IS DISTINCT FROM round(fv_variance_mean::numeric, 4)) $w$, dest_qual) USING latest_id;
      GET DIAGNOSTICS rcnt = ROW_COUNT;
    IF enable_full_analyze THEN EXECUTE format('ANALYZE %s', dest_qual); END IF;
...
-- Optional physical maintenance (A: gated)
  ...
  EXECUTE format('UPDATE engine.forecast_registry SET msq_complete = %s WHERE forecast_id = $1', quote_literal('complete'))
  USING latest_id;
END$$;
GRANT EXECUTE ON FUNCTION engine.msq_forecast(text) TO matrix_reader, tsf_engine_app;
GRANT EXECUTE ON FUNCTION engine.msq_forecast__core() TO matrix_reader, tsf_engine_app;