...
-- 2025-09-27: FIX v4 — removed all references to __msqm_subset to eliminate 42P01.
-- 2025-09-27: FIX v3 — removed all executable references to __msq_subset.
-- 2025-09-27: FIX — removed stray ANALYZE __msq_subset to prevent 42P01.
-- 2025-09-27: Scope lock — removed __ms_subset/__msq_subset and their filters; normalized index name to _sqm_yqm; kept only QM indexes in __work.
...
-- Entry point: engine.msqm_forecast(forecast_name TEXT)
...
IF EXISTS (SELECT 1 FROM pg_attribute WHERE attrelid='__work'::regclass AND attname='s_qm' AND NOT attisdropped) THEN
      EXECUTE 'CREATE INDEX IF NOT EXISTS __work_sqm_idx ON __work (s_qm)';
    END IF;
...
$f$,
    ...
    EXECUTE format('CREATE INDEX IF NOT EXISTS %I ON %s (%I, model_name, %I)',
                   dest_rel || '_sqm_yqm', dest_qual, dest_series_col, dest_season_col);
    IF enable_full_analyze THEN EXECUTE format('ANALYZE %s', dest_qual); END IF;
...
RAISE NOTICE 'COMPLETE series: % (elapsed %.3f s)',
  ...
  END LOOP;
  -- Mark completion in forecast_registry (msqm_complete) — type-aware
  BEGIN
    PERFORM 1
  ...
  EXECUTE format('UPDATE engine.forecast_registry SET msqm_complete = %s WHERE forecast_id = $1', quote_literal('complete'))
  USING latest_id;
