...
-- 2025-09-27: Hard-locked MS to *_instance_sr_s only; removed __work s_q/s_qm optional indexes; fixed index name to _s_yqm.
...
IF to_regclass('__ih_subset') IS NOT NULL THEN
  ...
  END IF;
  IF to_regclass('__msqm_subset') IS NOT NULL THEN
    EXECUTE 'CREATE TEMP TABLE __msqm_subset_filtered ON COMMIT DROP AS SELECT s.* FROM __msqm_subset s JOIN __universe u USING (forecast_id, date)';
    EXECUTE 'DROP TABLE __msqm_subset';
    EXECUTE 'ALTER TABLE __msqm_subset_filtered RENAME TO __msqm_subset';
    EXECUTE 'CREATE INDEX ON __msqm_subset (forecast_id, date)';
    EXECUTE 'ANALYZE __msqm_subset';
  END IF;
END;
...
$f$,
    ...
    -- index to support 6A
    EXECUTE format('CREATE INDEX IF NOT EXISTS %I ON %s (%I, model_name, %I)', dest_rel || '_s_yqm', dest_qual, dest_series_col, dest_season_col);
    IF enable_full_analyze THEN EXECUTE format('ANALYZE %s', dest_qual); END IF;
...
*/
-- v12: single end-of-run write to real destination
PERFORM set_config('synchronous_commit','off',true);
